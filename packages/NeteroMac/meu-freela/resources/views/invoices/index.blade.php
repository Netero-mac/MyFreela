    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-center">
            ...
            <form method="GET" action="{{ route('invoices.index') }}" class="w-full sm:w-1/2 lg:w-1/3">
                <div class="flex">
                    <x-text-input type="search" name="search" class="w-full" placeholder="Buscar por projeto ou cliente..." :value="request('search')" />
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100 overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                {{-- Colunas ajustadas para a nova realidade --}}
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Data de Anexo</th>
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse ($invoices as $invoice)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $invoice->project->title ?? 'N/A' }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        ...
                                        @if ($invoice->status)
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $invoice->status === \App\Enums\InvoiceStatus::PAID ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        {{-- O link de download agora baixa o PDF anexado --}}
